...
async function downloadPage(params) {
    ...
    content = params.title + content.replace(/ads.*?\);/g, '')
        // .replace(/爱去小说网 www.aiquxs.com，最快更新全职法师 ！/, '')
        .replace(/爱去小说网 www.aiquxs.com，最快更新鬼吹灯 ！/g, '')
        .replace(/喜欢本作品的朋友们请砸票、收藏本书吧，谢谢。/g, '')
        .replace(/     －－－－－－－－－－－－－－－－/g, '');
...
module.exports = {
    loader: async function(url) {
        ...
        let data = [];
        $("#list center").nextAll().find('a').each(function() {
            let node = $(this);
